...
class CancelUI(tk.Toplevel):
    ...
    def _build(self):
        ...
        bar.pack(fill="x")
        tk.Label(bar, text="❌  Cancel a Booking",
                 font=("Helvetica", 13, "bold"),
                 bg=PALETTE["accent"], fg="white").pack(padx=16, anchor="w")
        ...
        tk.Label(inner, text="Enter Booking Reference",
                 font=FONT_TITLE, bg=PALETTE["surface"],
                 fg=PALETTE["text"]).pack(anchor="w")
        tk.Label(inner, text="e.g. HCB-A1B2C3D4",
                 font=FONT_LABEL, bg=PALETTE["surface"],
                 fg=PALETTE["muted"]).pack(anchor="w", pady=(2, 16))

        ref_frame = tk.Frame(inner, bg=PALETTE["surface"])
        ref_frame.pack(fill="x")
        self._ref_var = tk.StringVar()
        entry = ttk.Entry(ref_frame, textvariable=self._ref_var,
                          font=FONT_INPUT)
    ...
    def _lookup(self):
        ...
        if eligible:
            self._status.config(
                text=f"✅ Eligible for cancellation. Refund: £{refund:.2f}",
                fg=PALETTE["success"])
            self._cancel_btn.pack(fill="x", pady=(12, 0), ipady=8)
        else:
            if booking.status == "Cancelled":
                msg = "⚠️ This booking has already been cancelled."
            else:
    ...
    def _confirm_cancel(self):
        ...
        try:
            ...
            self._cancel_btn.pack_forget()
            self._status.config(text=f"✅ Cancellation processed. "
