...
class BookingUI(tk.Toplevel):
    ...
    def _build(self):
        ...
        tk.Label(
            bar,
            text="🎬  Horizon Cinemas — New Booking",
        ...
        paned = tk.PanedWindow(
            self, orient="horizontal", bg=PALETTE["bg"], sashwidth=4, sashrelief="flat"
        )
    ...
    def _build_selection_panel(self, parent):
        ...
        self._film_cb = ttk.Combobox(
            f1, textvariable=self._film_var, state="readonly", font=FONT_INPUT, width=34
        )
        ...
        self._btn(
            btn_frame, "❌  Cancel Booking", self._open_cancel, bg=PALETTE["accent"]
        )
    ...
    def _build_seat_panel(self, parent):
        ...
        self._seat_canvas = tk.Canvas(
            canvas_frame, bg=PALETTE["surface"], highlightthickness=0
        )
        vsb = ttk.Scrollbar(
            canvas_frame, orient="vertical", command=self._seat_canvas.yview
        )
        ...
        self._seat_inner.bind(
            "<Configure>",
            lambda e: self._seat_canvas.configure(
    ...
    def _on_film_select(self, _=None):
        ...
        self._cinemas_for_film = {
            f"{r['city']} — {r['name']}": r["cinemaId"] for r in rows
        }
