...
class ManagerUI(tk.Toplevel):
    ...
    def _build_cinemas_tab(self):
        ...
        self._btn(
            tb, "🔄 Refresh", self._load_cinemas, bg=PALETTE["accent2"], side="right"
        )
    ...
    def _add_cinema_dialog(self):
        _FormDialog(
            self,
            title="Add Cinema",
            fields=[("Name", "name"), ("City", "city"), ("Address", "address")],
    ...
    def _build_cities_tab(self):
        ...
        self._btn(
            tb, "🔄 Refresh", self._load_cities, bg=PALETTE["accent2"], side="right"
        )
    ...
    def _build_staff_tab(self):
        ...
        self._btn(
            tb, "🔄 Refresh", self._load_staff, bg=PALETTE["accent2"], side="right"
        )
    ...
    def _build_pricing_tab(self):
        ...
        self._btn(
            tb,
            "✏️ Update Selected Price",
            self._update_price_dialog,
            bg=PALETTE["accent2"],
        )
        self._btn(
...
class _FormDialog(tk.Toplevel):
    ...
    def __init__(self, master, title: str, fields: list, on_submit, on_success=None):
        super().__init__(master)
        self._fields = fields
    ...
    def _build(self):
        ...
        for i, (label, key) in enumerate(self._fields):
            ...
            ttk.Entry(
                inner, textvariable=var, show=show, font=FONT_INPUT, width=28
            ).grid(row=i, column=1, padx=(12, 0), pady=6, sticky="ew")
        ...
        btn_f.grid(
            row=len(self._fields) + 1, column=0, columnspan=2, sticky="ew", pady=(10, 0)
        )
        btn = tk.Label(
            btn_f,
            text="SAVE",
