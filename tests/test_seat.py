...
def test_seat_from_row():
    row = {"seatId": 1, "screenId": 10, "seatNumber": "A1", "seatType": SeatType.LOWER_HALL}
    seat = Seat.from_row(row)
    assert seat.seatId == 1

@patch("models.seat.db")
def test_seat_get_by_id(mock_db):
    mock_db.fetchone.return_value = {"seatId": 1, "screenId": 10, "seatNumber": "A1", "seatType": SeatType.LOWER_HALL}
...
def test_seat_get_by_screen(mock_db):
    mock_db.fetchall.return_value = [
        {"seatId": 1, "screenId": 10, "seatNumber": "A1", "seatType": SeatType.LOWER_HALL},
        {"seatId": 2, "screenId": 10, "seatNumber": "A2", "seatType": SeatType.LOWER_HALL}
    ]
    seats = Seat.get_by_screen(10)
    assert len(seats) == 2

@pytest.mark.parametrize("seat_type, base_price, expected", [
