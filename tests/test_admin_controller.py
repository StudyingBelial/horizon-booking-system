...
def test_add_listing_success(mock_listing_get, mock_screen_get, mock_film_get, mock_db, admin_controller):
...
def test_update_listing_success(mock_db, mock_listing_get, admin_controller):
    mock_listing_get.side_effect = [MagicMock(listingId=1), MagicMock(listingId=1, showType="3D")]
    
    listing = admin_controller.update_listing(1, show_type="3D")
    
    assert listing.showType == "3D"
    assert "UPDATE listings SET showType=? WHERE listingId=?" in mock_db.execute.call_args[0][0]
...
def test_remove_listing_active_bookings(mock_db, admin_controller):
    ...
    with pytest.raises(ValidationError, match="Cannot remove a listing that has active confirmed bookings."):
...
def test_generate_report(mock_init, admin_controller):
    ...
    admin_controller._reports.generate_and_export.return_value = ({"data": "report"}, "/path/to/file")
    
    admin_controller.generate_report("sales", export=True, filepath="test.csv")
    admin_controller._reports.generate_and_export.assert_called_once_with("sales", "test.csv")
