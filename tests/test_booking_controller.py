...
def test_create_booking_success(mock_booked_seat, mock_booking, mock_seat, mock_screen, mock_listing, booking_controller):
    mock_listing.return_value = MagicMock(showDate="2026-05-10", screenId=1, showType="Standard")
...
def test_cancel_booking_success(mock_refund, mock_cancel_create, mock_booking_get, booking_controller):
    ...
    with patch.object(booking_controller._validation, "validate_cancellation_eligibility"):
...
def test_cancel_booking_not_found(mock_booking_get, booking_controller):
    ...
    with pytest.raises(ValidationError, match="No booking found with reference 'REF123'."):
