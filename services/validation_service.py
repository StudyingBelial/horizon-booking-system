...
class ValidationService:
    ...
    def validate_booking_date(self, show_date_str: str) -> None:
        ...
        try:
            ...
            raise ValidationError(
                f"Invalid date format: {show_date_str!r}. " f"Expected YYYY-MM-DD."
            )
        today = date.today()
    ...
    def validate_seat_availability(
        self, screen, listing_id: int, seat_ids: list
    ) -> None:
        """Raise if any requested seat is already booked."""
        unavailable = [
            sid for sid in seat_ids if not screen.checkSeatAvailability(sid, listing_id)
        ]
        if unavailable:
            raise ValidationError(
